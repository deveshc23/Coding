...
class FedAvgServer:
    ...
    def aggregate(self, client_updates):
        ...
        self.round += 1
        log.info(
            f"Round {self.round} aggregated | {len(client_updates)} clients | {total_samples} total samples"
        )
    ...
    def evaluate(self, test_loader):
        ...
        total_loss = 0.0
        total_correct = 0
        total_samples = 0  # fix: was incorrectly named total_tokens (this is image classification, not NLP)
        with torch.no_grad():
            for seqs, tgts in test_loader:
                seqs, tgts = seqs.to(self.device), tgts.to(self.device)
                logits = self.global_model(seqs)
        ...
        # fix: removed undefined `perplexity` variable — not applicable to image classification
        log.info(
            f"Round {self.round} eval | loss={avg_loss:.4f} | acc={accuracy:.4f}"
        )
        return {"loss": avg_loss, "accuracy": accuracy}

    def save_checkpoint(self, path):
        torch.save(
            {"round": self.round, "model_state": self.global_model.state_dict()}, path
        )
